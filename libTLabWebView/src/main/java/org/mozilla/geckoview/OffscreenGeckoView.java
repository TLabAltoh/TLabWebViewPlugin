...
@UiThread
public class OffscreenGeckoView extends FrameLayout implements GeckoDisplay.NewSurfaceProvider {
    private static final String TAG = "OffscreenGeckoView";
    ...
    private final ViewToBufferRenderer mViewToBufferRenderer;
    private final BaseOffscreenFragment.CaptureMode mCaptureMode;
    ...
    private class Display implements SurfaceHolder.Callback {
        ...
        public void acquire(final GeckoDisplay display) {
            ...
            onGlobalLayout();
            Surface surface = mViewToBufferRenderer.getSurface();
            Vector2Int size = mViewToBufferRenderer.getViewSize();
            if (surface != null) {
                mDisplay.surfaceChanged(new GeckoDisplay.SurfaceInfo.Builder(surface).newSurfaceProvider(OffscreenGeckoView.this).size(size.x, size.y).build());
                mDisplay.setDynamicToolbarMaxHeight(mDynamicToolbarMaxHeight);
                OffscreenGeckoView.this.setActive(true);
            }
        }
        ...
        public GeckoDisplay release() {
            if (mValid) {
                ...
                OffscreenGeckoView.this.setActive(false);
            }
        ...
        public void onGlobalLayout() {
            ...
            if ((mCaptureMode != BaseOffscreenFragment.CaptureMode.SURFACE) && (mSurfaceView != null)) {
                ...
                if (Build.VERSION.SDK_INT >= 28) {
                    final DisplayCutout cutout = OffscreenGeckoView.this.mSurfaceView.getRootWindowInsets().getDisplayCutout();
                    if (cutout != null)
        ...
        public void setVerticalClipping(final int clippingHeight) {
            ...
            if (mDisplay != null) mDisplay.setVerticalClipping(clippingHeight);
        }
        ...
        public void setDynamicToolbarMaxHeight(final int height) {
            ...
            if (mDisplay != null) mDisplay.setDynamicToolbarMaxHeight(height);
        }
        ...
        public void surfaceChanged(GeckoDisplay.SurfaceInfo build) {
            if (mDisplay != null) {
                mDisplay.surfaceChanged(build);
                mDisplay.setDynamicToolbarMaxHeight(mDynamicToolbarMaxHeight);
                if (!mValid) OffscreenGeckoView.this.setActive(true);
            }
            mValid = true;
        }
    }
    ...
    public void onSurfaceChanged(Surface surface, int width, int height) {
        new Handler(Looper.getMainLooper()).post(() -> mDisplay.surfaceChanged(new GeckoDisplay.SurfaceInfo.Builder(surface).newSurfaceProvider(OffscreenGeckoView.this).size(width, height).build()));
    }

    public void removeSurface() {
        new Handler(Looper.getMainLooper()).post(mDisplay::removeSurface);
    }

    public OffscreenGeckoView(final Context context, BaseOffscreenFragment.CaptureMode captureMode, final ViewToBufferRenderer renderer) {
        super(context);
    ...
    public OffscreenGeckoView(final Context context, final AttributeSet attrs, BaseOffscreenFragment.CaptureMode captureMode, @Nullable final ViewToBufferRenderer renderer) {
        super(context, attrs);
        mCaptureMode = captureMode;
        mViewToBufferRenderer = renderer;
        if ((mViewToBufferRenderer != null) && (mCaptureMode != BaseOffscreenFragment.CaptureMode.SURFACE))
    ...
    private void init() {
        ...
        if (mCaptureMode != BaseOffscreenFragment.CaptureMode.SURFACE) {
            ...
            surfaceView.setRenderer(mViewToBufferRenderer);
        ...
        if (activity != null) mSelectionActionDelegate = new BasicSelectionActionDelegate(activity);
    ...
    public void setViewBackend(final @ViewBackend int backend) {
        ...
        if (mSession != null) {
            mSession.getMagnifier().setView(mSurfaceView);
        }
    }
    ...
    private int defaultColor() {
        ...
        return Objects.requireNonNull(mSession.getRuntime()).usesDarkTheme() ? DEFAULT_DARK_COLOR : Color.WHITE;
    }
    ...
    public @Nullable GeckoSession releaseSession() {
        ...
        if (mSession.getMagnifier().getView() == mSurfaceView) {
    ...
    private final GeckoSession.Owner mSessionOwner = new GeckoSession.Owner() {
        @Override
    ...
    public void setSession(@NonNull final GeckoSession session) {
        ...
        session.getOverscrollEdgeEffect().setInvalidationCallback(OffscreenGeckoView.this::postInvalidateOnAnimation);
        ...
        if (session.getMagnifier().getView() == null) {
            session.getMagnifier().setView(mSurfaceView);
        }
    ...
    protected void onFocusChanged(final boolean gainFocus, final int direction, final Rect previouslyFocusedRect) {
        ...
        post(() -> {
            ...
            if (!imm.isActive(OffscreenGeckoView.this)) {
    ...
    public @NonNull GeckoResult<PanZoomController.InputResultDetail> onTouchEventForDetailResult(final @NonNull MotionEvent event) {
        ...
        if (mSession == null) {
            return GeckoResult.fromValue(new PanZoomController.InputResultDetail(PanZoomController.INPUT_RESULT_UNHANDLED, PanZoomController.SCROLLABLE_FLAG_NONE, PanZoomController.OVERSCROLL_FLAG_NONE));
        }
    ...
    private class AndroidAutofillDelegate implements Autofill.Delegate {
        ...
        private void ensureAutofillManager() {
            ...
            mAutofillManager = OffscreenGeckoView.this.getContext().getSystemService(AutofillManager.class);
        ...
        private Rect displayRectForId(@NonNull final GeckoSession session, @Nullable final Autofill.Node node) {
            if (node == null) {
        ...
        @Override
        public void onNodeBlur(final @NonNull GeckoSession session, final @NonNull Autofill.Node prev, final @NonNull Autofill.NodeData data) {
            ensureAutofillManager();
            if (mAutofillManager == null) {
                return;
            }
            try {
                mAutofillManager.notifyViewExited(OffscreenGeckoView.this, data.getId());
            } catch (final SecurityException e) {
                Log.e(TAG, "Failed to call AutofillManager.notifyViewExited: ", e);
            }
        }

        @Override
        public void onNodeAdd(final @NonNull GeckoSession session, final @NonNull Autofill.Node node, final @NonNull Autofill.NodeData data) {
            assert mSession != null;
            ...
            try {
                mAutofillManager.notifyViewExited(OffscreenGeckoView.this, focusedData.getId());
                mAutofillManager.notifyViewEntered(OffscreenGeckoView.this, focusedData.getId(), displayRectForId(session, focused));
            } catch (final SecurityException e) {
                Log.e(TAG, "Failed to call AutofillManager.notifyViewExited or AutofillManager.notifyViewEntered: ", e);
            }
        }

        @Override
        public void onNodeFocus(final @NonNull GeckoSession session, final @NonNull Autofill.Node focused, final @NonNull Autofill.NodeData data) {
            ensureAutofillManager();
            if (mAutofillManager == null) {
                return;
            }
            try {
                mAutofillManager.notifyViewEntered(OffscreenGeckoView.this, data.getId(), displayRectForId(session, focused));
            } catch (final SecurityException e) {
                Log.e(TAG, "Failed to call AutofillManager.notifyViewEntered: ", e);
            }
        }

        @Override
        public void onNodeRemove(final @NonNull GeckoSession session, final @NonNull Autofill.Node node, final @NonNull Autofill.NodeData data) {
        }

        @Override
        public void onNodeUpdate(final @NonNull GeckoSession session, final @NonNull Autofill.Node node, final @NonNull Autofill.NodeData data) {
            ensureAutofillManager();
            if (mAutofillManager == null) {
                return;
            }
            try {
                mAutofillManager.notifyValueChanged(OffscreenGeckoView.this, data.getId(), AutofillValue.forText(data.getValue()));
            } catch (final SecurityException e) {
                Log.e(TAG, "Failed to call AutofillManager.notifyValueChanged: ", e);
            }
        }
        ...
        public void onSessionCancel(final @NonNull GeckoSession session) {
            ...
            try {
                ...
                Log.e(TAG, "Failed to call AutofillManager.cancel: ", e);
            }
        }

        @Override
        public void onSessionCommit(final @NonNull GeckoSession session, final @NonNull Autofill.Node node, final @NonNull Autofill.NodeData data) {
            ensureAutofillManager();
            if (mAutofillManager == null) {
                return;
            }
            try {
                ...
                Log.e(TAG, "Failed to call AutofillManager.commit: ", e);
            }
        }
    ...
    private class GeckoViewPrintDelegate implements GeckoSession.PrintDelegate {
        public void onPrint(@NonNull final GeckoSession session) {
            ...
            geckoResult.accept(pdfStream -> {
                assert pdfStream != null;
                onPrint(pdfStream);
            }, exception -> Log.e(TAG, "Could not create a content PDF to print.", exception));
        }
        ...
        public GeckoResult<Boolean> onPrintWithStatus(@NonNull final InputStream pdfStream) {
            ...
            if (mActivityDelegate == null) {
                Log.w(TAG, "Missing an activity context delegate, which is required for printing.");
                isDialogFinished.completeExceptionally(new GeckoSession.GeckoPrintException(ERROR_NO_ACTIVITY_CONTEXT_DELEGATE));
                return isDialogFinished;
            }
            final Context printContext = mActivityDelegate.getActivityContext();
            if (printContext == null) {
                Log.w(TAG, "An activity context is required for printing.");
                isDialogFinished.completeExceptionally(new GeckoSession.GeckoPrintException(ERROR_NO_ACTIVITY_CONTEXT));
                return isDialogFinished;
            }
            final PrintManager printManager = (PrintManager) mActivityDelegate.getActivityContext().getSystemService(Context.PRINT_SERVICE);
            final PrintDocumentAdapter pda = new GeckoViewPrintDocumentAdapter(pdfStream, getContext(), isDialogFinished);
